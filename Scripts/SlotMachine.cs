...
public class SlotMachine : MonoBehaviour
{
    ...
    [Header("Chip Display")]
    public Text chipDisplayText;          // Live display of Dave's chips
    ...
    IEnumerator Spin()
    {
        if (dave.chips < spinCost)
        {
            resultText.text = "Not enough chips!";
            yield break;
        }
    ...
    void EvaluateResult()
    {
        ...
        foreach (var line in paylines)
        {
            ...
            {
                winnings += 20; // Flat payout per line win
            }
        }

        if (winnings > 0)
        {
            dave.AddChips(winnings);
            resultText.text = $"You won {winnings} chips!";
        }
        else
        {
    ...
    {
        if (dave != null && chipDisplayText != null)
        {
            chipDisplayText.text = $"Chips: {dave.chips}";
        }
    }
